...
export default function OptionSelector() {
  ...
  return <View style={{ marginBottom: 16 }}>
    <Pressable accessibilityRole="button" accessibilityLabel={`Active option: ${active.name}`} onPress={() => setOpen(true)}
      style={{ minHeight: 48, padding: 12, borderWidth: 1, borderColor: colors.border, borderRadius: 14, flexDirection: 'row', alignItems: 'center', gap: 12 }}>
    ...
    <Modal visible={open} transparent animationType="none" onRequestClose={() => setOpen(false)}>
      <View style={{ flex: 1, justifyContent: 'center', padding: 24, backgroundColor: '#00000080' }}>
        <View accessibilityViewIsModal style={{ width: '100%', maxWidth: 520, maxHeight: '80%', alignSelf: 'center', backgroundColor: colors.surface, padding: 20, borderRadius: 20, gap: 16 }}>
